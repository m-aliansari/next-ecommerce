						...
						<div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
											...
											return Object.keys(variants[c]).includes(size) ? (
												<button
													...
													className={`border-2  ml-1 rounded-full w-6 h-6 focus:outline-none ${
														c === color ? "border-black" : "border-gray-200"
													}`}
							...
							<div className="flex">
								...
								<button
									...
									className="flex ml-8 text-white bg-gray-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-gray-600 rounded"
								>
									Add to Cart
								</button>
							...
							<div className="pin mt-6 flex space-x-2 text-sm">
								<input
									...
									value={pin}
									className="px-2 border-2 border-gray-300 rounded-md focus:outline-none focus:border-gray-500"
									type="number"
								/>
								<button
									onClick={checkServiceability}
									className="flex ml-14 text-white bg-gray-500 border-0 py-2 px-6 focus:outline-none hover:bg-gray-600 rounded"
								>
									Check
								</button>
							</div>
							{service !== undefined && !service && (
								<div className="text-sm mt-3 text-red-700">
...
export async function getServerSideProps(context) {
	...
	for (const item of variants) {
		if (Object.keys(colorSizeSlug).includes(item.color.toLowerCase())) {
			colorSizeSlug[item.color.toLowerCase()][item.size.toUpperCase()] = {
				slug: item.slug,
			};
		} else {
			colorSizeSlug[item.color.toLowerCase()] = {};
			colorSizeSlug[item.color.toLowerCase()][item.size.toUpperCase()] = {
				slug: item.slug,
			};
		}
